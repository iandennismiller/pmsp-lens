...
function run_one {
    PMSP_RANDOM_SEED=$1 \
        PMSP_DILUTION=$2 \
        PMSP_PARTITION=$3 \
        /opt/Lens-linux/Bin/alens.sh \
        $4
}

function run_one_fig18 {
    run_one $1 $2 $3 scripts/activations-fig18-dt-100.tcl
}

function run_one_anchors {
    run_one $1 $2 $3 scripts/activations-anchors-dt-100.tcl
}

function run_one_probes {
    run_one $1 $2 $3 scripts/activations-probes-dt-100.tcl
}
